class DBStub:
    def __init__(self, f, dbname='example.db', tableName='objects', callback=None):
        ...
        self.__createTable(tableName, ["id INTEGER PRIMARY KEY   AUTOINCREMENT", "x real", "y real", "z real", "yaw real", "path text"], callback)
    ...
    def addObject(self, position, yaw, path, callback=None):
        ...
        request = {'method':'execute', 'args':[query, (None, x, y, z, yaw, path)]}
        self.__send(request, callback)
        
    def removeObject(self, dbId, callback=None):
        query = "DELETE FROM " + self.tableName + " WHERE id=?"
        request = {'method':'execute', 'args':[query, (dbId)]}
        self.__send(request, callback)
    ...
    def getObject(self, dbId, callback): 
        ...
        request = {'method':'executeFetchOne', 'args':[query, (dbId)]}
        self.__send(request, callback)
    ...
    def getLastId(self, callback):
        ...
        request = {'method':'executeFetchOne', 'args':[query]}
        self.__send(request, callback)
